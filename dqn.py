...
def train(model, start):
    ...
    while iteration < model.number_of_iterations:
        ...
        command = 'UscCmd --sub ' + str(action_index.item())
        os.system(command)
        time.sleep(0.18)
...
def play_game(model):
    ...
    while True:
        try:  # used try so that if user pressed other than the given key error will not be shown
            if keyboard.is_pressed('a'):  # if key 'q' is pressedd
                action[0] = 1 #
    ...
    while True:
        try:  # used try so that if user pressed other than the given key error will not be shown
            ...
            if keyboard.is_pressed('s'):  # if key 'q' is pressed
                ...
                if (action_index == 0):
                    pyautogui.press('a')
                    print("simuleer a klik")
                if (action_index == 1):
                    pyautogui.press('d')
                    print("simuleer d klik")
        except:
            output = model(state)[0]
            print(output)
            action_index = torch.argmax(output)
            ...
            if (action_index == 0):
                pyautogui.press('a')
                print("2. simuleer a klik")
            if (action_index == 1):
                pyautogui.press('d')
                print("2. simuleer d klik")
            #break
...
def main(mode):
    if mode == 'test':
        ...
        model = torch.load('trained_model/current_model_420000.pth', map_location='cpu').eval()
        play_game(model)
    elif mode == 'modelpress':
        model = torch.load('trained_model/current_model_420000.pth', map_location='cpu').eval()
        play_game_model(model)
